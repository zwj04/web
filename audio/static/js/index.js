(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["index"],{
  ...
  /* WEBPACK VAR INJECTION */(function($) {/* harmony import */ var crypto_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! crypto-js */ "./node_modules/_crypto-js@4.1.1@crypto-js/index.js");
  ...
  /* harmony import */ var vconsole__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! vconsole */ "./node_modules/_vconsole@3.12.0@vconsole/dist/vconsole.min.js");
  ...
  var APPID = 'cd03c130';
  var API_SECRET = 'ZGJhMmEyMmQwOTFlYTkzZjkwMmNjZTU0';
  var API_KEY = '2d627eb00c761e1cba9a6b3af3ac9f50';
  /**
   * 获取websocket url
  ...
  function show_cmd(txt)
  {
    var new_txt = txt;
  
    cmd_item_1 = cmd_item_2;
    cmd_item_2 = cmd_item_3;
    cmd_item_3 = new_txt;
  
    $('#result_cmd_item').html('-->'+ cmd_item_3+'<--' +'<br>'+cmd_item_2+'<br>'+cmd_item_1+'<br>');
  }
  
  function parse_txt_cmd(var_txt) {
  
    var search_str = var_txt ;
    var search_table = value_sensor_table;
  
    if(value_sensor_table.length == 0)
    {
      console.log("not get sensor table");
      return ;
    }
  
    console.log("entry parse_txt_cmd:"+var_txt);
  
    for (let item of search_table) {
  
      if (item.sensor_role == "switch") {
  
        console.log("enty switch: item.name:" + item.name);
  
        if(search_str.indexOf(item.sensor_name) != -1)
        {
          if(search_str.indexOf('开') != -1)
          {
            show_cmd('【'+item.sensor_name+'】'+'【开】');
            item.sensor_value = 'on';
            request_option("update", item, function (data) {});
          }
          else if(search_str.indexOf('关') != -1)
          {
            show_cmd('【'+item.sensor_name+'】'+'【关】');
  ...
  function analyze_txt(text)
  {
    if(text.length > temp_text.length)
    {
      var text_value = text;
      var cmd_item = text_value.substring(temp_text.length);
      console.log("cmd item:"+cmd_item);
      parse_txt_cmd(cmd_item);
    }
    else
    {
      parse_txt_cmd(text);
